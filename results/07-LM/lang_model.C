...
void LangModel::count_sentence_ngrams(const vector<int>& wordList) {
  ...
  for (int n = 1; n <= m_n; n++) {
    for (int i = m_n - n; i < wordCnt - n + 1; i++) {
      vector<int> ngram;
      for (int j = 0; j < n; j++) {
        ngram.push_back(wordList[i + j]);     
      }
      int cnt = m_predCounts.incr_count(ngram);
      ngram.pop_back();
...
double LangModel::get_prob_witten_bell(const vector<int>& ngram) const {
  ...
  for (int n = 0; n < ngramSize; n++) {
    vector<int> histGram(ngram.end() - n - 1, ngram.end() - 1);
    int histCnt = m_histCounts.get_count(histGram);

    if (histCnt == 0)
        continue;

    vector<int> predGram(ngram.end() - n - 1, ngram.end());
    int predCnt = m_predCounts.get_count(predGram);
    double pMLE = 1.0 * predCnt / histCnt;

    int histOnePlusCnt = m_histOnePlusCounts.get_count(histGram);
    retProb =
        1.0 * histCnt / (histCnt + histOnePlusCnt) * pMLE +
        1.0 * histOnePlusCnt / (histCnt + histOnePlusCnt) * retProb;
  }
  return retProb;
}
