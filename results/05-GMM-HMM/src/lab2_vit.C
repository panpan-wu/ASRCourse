...
double viterbi(const Graph& graph, const matrix<double>& gmmProbs,
  ...
  for (int frmIdx = 0; frmIdx < frmCnt; ++frmIdx) {
      for (int stateIdx = 0; stateIdx < stateCnt; ++stateIdx) {
        if (chart(frmIdx, stateIdx).get_log_prob() == g_zeroLogProb)
            continue;
        ...
        for (int arcIdx = 0; arcIdx < arcCnt; ++arcIdx) {
            ...
            double prob =
                chart(frmIdx, stateIdx).get_log_prob() +
                arc.get_log_prob() +
                gmmProbs(frmIdx, arc.get_gmm());
            int dstStateIdx = arc.get_dst_state();
            if (prob > chart(frmIdx + 1, dstStateIdx).get_log_prob()) {
                chart(frmIdx + 1, dstStateIdx).assign(prob, arcId - 1);
            }
        }
      }
  }
