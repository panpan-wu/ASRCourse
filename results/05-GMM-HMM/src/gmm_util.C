...
double GmmStats::add_gmm_count(unsigned gmmIdx, double posterior,
  ...
  for (int dimIdx = 0; dimIdx < dimCnt; dimIdx++) {
      m_gaussStats1(gaussIdx, dimIdx) += feats[dimIdx] * posterior;
      m_gaussStats2(gaussIdx, dimIdx) += feats[dimIdx] * feats[dimIdx] * posterior;
  }
...
void GmmStats::reestimate() const {
  ...
  for (int gaussIdx = 0; gaussIdx < gaussCnt; gaussIdx++) {
    double sumOfPosterior = m_gaussCounts[gaussIdx];
    for (int dimIdx = 0; dimIdx < dimCnt; dimIdx++) {
       double newMean = m_gaussStats1(gaussIdx, dimIdx) / sumOfPosterior;
       double newVar = m_gaussStats2(gaussIdx, dimIdx) / sumOfPosterior - newMean * newMean;
       m_gmmSet.set_gaussian_mean(gaussIdx, dimIdx, newMean);
       m_gmmSet.set_gaussian_var(gaussIdx, dimIdx, newVar);
    }
  }
