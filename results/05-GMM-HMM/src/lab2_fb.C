...
double forward_backward(const Graph& graph, const matrix<double>& gmmProbs,
  ...
  for (int frmIdx = 0; frmIdx < frmCnt; frmIdx++) {
    vector<vector<double> > forwardProbs(stateCnt);
    for (int stateIdx = 0; stateIdx < stateCnt; stateIdx++) {
        if (chart(frmIdx, stateIdx).get_forw_log_prob() == g_zeroLogProb)
            continue;
        ...
        for (int arcIdx = 0; arcIdx < arcCnt; arcIdx++) {
            ...
            double prob =
                chart(frmIdx, stateIdx).get_forw_log_prob() +
                arc.get_log_prob() +
                gmmProbs(frmIdx, arc.get_gmm());
            forwardProbs[arc.get_dst_state()].push_back(prob);
        }
    }
    for (int stateIdx = 0; stateIdx < stateCnt; ++stateIdx) {
      if (forwardProbs[stateIdx].size() > 0) {
        chart(frmIdx + 1, stateIdx).set_forw_log_prob(add_log_probs(forwardProbs[stateIdx])); 
      }
    }
  }
  ...
  for (int frmIdx = frmCnt - 1; frmIdx >= 0; frmIdx--) {
    ...
    for (int stateIdx = 0; stateIdx < stateCnt; stateIdx++) {
      ...
      for (int arcIdx = 0; arcIdx < arcCnt; arcIdx++) {
        Arc arc;
        arcId = graph.get_arc(arcId, arc);
        double backLogProb = chart(frmIdx + 1, arc.get_dst_state()).get_back_log_prob();

        if (backLogProb == g_zeroLogProb)
            continue;
        ...
        double prob =
            ...
            arc.get_log_prob() +
            gmmProbs(frmIdx, arc.get_gmm());
        backwardProbs[stateIdx].push_back(prob);

        double arcPosterior = exp(
            chart(frmIdx, stateIdx).get_forw_log_prob() +
            arc.get_log_prob() +
            gmmProbs(frmIdx, arc.get_gmm()) +
            backLogProb -
