...
class GMM:
    ...
    def calc_log_likelihood(self , X):
        ...
        for x in X:
            ...
            for pi, mu, sigma in zip(self.pi, self.mu, self.sigma):
                res += pi * self.gaussian(x, mu, sigma)
            log_llh += np.log(res)
        # end
        return log_llh
    ...
    def em_estimator(self , X):
        ...
        log_llh = 0.0
        # start
        r_znks = np.array([self._calc_r_znk(x) for x in X])
        n_ks = np.sum(r_znks, axis=0)
        mus = np.dot(r_znks.T, X)
        ...
        for k in range(mus.shape[0]):
            ...
            for n, x in enumerate(X):
                sigma += np.outer(x-mu, (x-mu).T) * r_znks[n][k]
            sigma /= n_ks[k]
            sigmas.append(sigma)
    ...
    def _calc_r_znk(self, x):
        """计算观测数据x的r_zn1,r_zn2,...,r_znk
            :param x: The observed data, dim*1.
        """
        res = []
        for pi, mu, sigma in zip(self.pi, self.mu, self.sigma):
            res.append(pi * self.gaussian(x, mu, sigma))
        p = sum(res)
        res = [e/p for e in res]
        return res
